...
function WaveCanvas({ opacity = 1 }: { opacity?: number }) {
  ...
  useEffect(() => {
    ...
    const WAVES = [{amp:20,freq:.011,spd:.016,y:.38,a:.07,lw:1.8},{amp:13,freq:.017,spd:.022,y:.52,a:.05,lw:1.2},{amp:24,freq:.008,spd:.012,y:.62,a:.04,lw:2.2},{amp:9,freq:.024,spd:.028,y:.30,a:.03,lw:1.0},{amp:16,freq:.013,spd:.018,y:.72,a:.03,lw:1.5}];
    const draw = () => {
      const W=canvas.width,H=canvas.height; ctx.clearRect(0,0,W,H); t++;
      WAVES.forEach(w=>{ctx.beginPath();ctx.moveTo(0,H*w.y);for(let x=0;x<=W;x+=2){const y=H*w.y+Math.sin(x*w.freq+t*w.spd)*w.amp+Math.sin(x*w.freq*1.7+t*w.spd*.6)*w.amp*.35;ctx.lineTo(x,y);}ctx.strokeStyle=`rgba(0,0,0,${w.a*opacity})`;ctx.lineWidth=w.lw;ctx.stroke();});
...
function NeuralCanvas() {
  ...
  useEffect(() => {
    ...
    const resize=()=>{canvas.width=canvas.offsetWidth;canvas.height=canvas.offsetHeight;pts=Array.from({length:32},()=>({x:Math.random()*canvas.width,y:Math.random()*canvas.height,vx:(Math.random()-.5)*.3,vy:-Math.random()*.35-.08,r:Math.random()*2+1,a:Math.random()*.09+.03}));};
    resize();window.addEventListener("resize",resize);
    const draw=()=>{const W=canvas.width,H=canvas.height;ctx.clearRect(0,0,W,H);for(let i=0;i<pts.length;i++)for(let j=i+1;j<pts.length;j++){const dx=pts[i].x-pts[j].x,dy=pts[i].y-pts[j].y,d=Math.sqrt(dx*dx+dy*dy);if(d<130){ctx.globalAlpha=((130-d)/130)*.05;ctx.strokeStyle="#111";ctx.lineWidth=.8;ctx.beginPath();ctx.moveTo(pts[i].x,pts[i].y);ctx.lineTo(pts[j].x,pts[j].y);ctx.stroke();}}pts.forEach(p=>{ctx.globalAlpha=p.a;ctx.fillStyle="#111";ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fill();p.x+=p.vx;p.y+=p.vy;if(p.y<-4){p.y=H+4;p.x=Math.random()*W;}if(p.x<0||p.x>W)p.vx*=-1;});ctx.globalAlpha=1;id=requestAnimationFrame(draw);};
    draw();return()=>{cancelAnimationFrame(id);window.removeEventListener("resize",resize);};
  },[]);
  return <canvas ref={ref} className="absolute inset-0 w-full h-full pointer-events-none" style={{zIndex:0}} />;
}
      ...
      <motion.div className="space-y-0" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
        {beforeAfter.map((row,i)=>(
          <motion.div key={row.before} variants={staggerItem}
            ...
            <motion.div className="absolute inset-0 bg-black/[0.025]" initial={{scaleX:0,originX:0}}
              animate={{scaleX:active===i?1:0}} transition={{duration:.3,ease:[.16,1,.3,1]}} />
            <div className="flex items-center gap-2 relative">
              <motion.span className="text-sm" animate={{color:active===i?"rgba(0,0,0,0.25)":"rgba(0,0,0,0.38)",x:active===i?-3:0}}
                style={{textDecoration:active===i?"line-through":"none",textDecorationColor:"rgba(0,0,0,0.2)"}} transition={{duration:.22}}>
                {row.before}
              </motion.span>
              <motion.span className="text-xs flex-shrink-0" animate={{color:active===i?"#111827":"rgba(0,0,0,0.2)",x:active===i?2:0}} transition={{duration:.2}}>→</motion.span>
              <motion.span className="text-sm" animate={{color:active===i?"#111827":"rgba(0,0,0,0.68)",fontWeight:active===i?500:400}} transition={{duration:.2}}>{row.after}</motion.span>
            ...
            <motion.div className="absolute bottom-0 left-0 h-px bg-black/30" animate={{scaleX:active===i?1:0}}
        ...
        {steps.map((s,i)=>(
          <div key={s} className="flex flex-col items-center gap-2" style={{flex:1,zIndex:2,position:"relative"}}>
            ...
            <motion.span className="text-[9px] text-center leading-tight"
              animate={{color:activeStep===i?"#111827":"rgba(0,0,0,0.42)",fontWeight:activeStep===i?600:400}}
        ...
        <motion.div className="grid md:grid-cols-[220px_1fr] border border-black/10 rounded-xl overflow-hidden"
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-12}}
              ...
              <h3 className="text-xl md:text-2xl font-normal text-black leading-snug mb-3 tracking-tight">
                {current.title}
              </h3>
              ...
              <div className="flex items-center justify-between mt-8 pt-6 border-t border-black/[0.06]">
                <span className="text-xs text-black/30">{active+1} of {aiOfferings.length}</span>
      ...
      <section className="py-24 px-4 bg-black/[0.02] border-t border-b border-black/10">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <motion.p className="text-base text-black/55 leading-relaxed mt-4"
