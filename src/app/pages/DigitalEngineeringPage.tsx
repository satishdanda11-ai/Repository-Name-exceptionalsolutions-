    ...
    <div className="mt-8 bg-white border border-black/10 rounded-lg px-7 py-6">
      ...
      <div className="relative flex items-start justify-between">
        {STEPS.map((s,i)=>(
          <div key={s} className="flex flex-col items-center gap-2" style={{flex:1,position:"relative",zIndex:2}}>
            <motion.div className="w-9 h-9 rounded-full border flex items-center justify-center text-[9px] font-medium"
              animate={{background:active===i?"#111827":"#fff",borderColor:active===i?"#111827":"rgba(0,0,0,0.14)",color:active===i?"#fff":"rgba(0,0,0,0.45)",boxShadow:active===i?"0 0 0 5px rgba(0,0,0,0.08)":"none"}}
        ...
        <div ref={trackRef} className="absolute" style={{top:17,left:"4%",right:"4%",height:2,background:"rgba(0,0,0,0.07)",zIndex:1}}>
          <motion.div className="absolute top-0 left-0 h-full bg-black/20" animate={{width:`${(active/(STEPS.length-1))*100}%`}} transition={{duration:.5,ease:"easeInOut"}} />
  ...
  return (
    <motion.div variants={staggerItem}
      ...
      <div className="relative">
        <motion.div className="text-2xl font-medium text-black tabular-nums mb-0.5" animate={{scale:hovered?1.04:1}} transition={{duration:.2}}>{item.metric}</motion.div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-12}}
              ...
              <div className="flex items-center justify-between mt-8 pt-6 border-t border-black/[0.06]">
                <span className="text-xs text-black/30">{active+1} of {digitalOfferings.length}</span>
                <div className="flex gap-3">
                  <button onClick={()=>setActive(a=>(a-1+digitalOfferings.length)%digitalOfferings.length)}
                  ...
                  <button onClick={()=>setActive(a=>(a+1)%digitalOfferings.length)}
                    className="flex items-center justify-center transition-all duration-200 hover:scale-105"
                    style={{width:38,height:38,borderRadius:"50%",background:"#111827",border:"none",cursor:"pointer"}}>
