...
function DataStreamCanvas() {
  ...
  useEffect(() => {
    ...
    const resize=()=>{canvas.width=canvas.offsetWidth;canvas.height=canvas.offsetHeight;streams=Array.from({length:22},(_,i)=>({x:(canvas.width/22)*i+Math.random()*40-20,y:Math.random()*canvas.height,vy:.4+Math.random()*.6,len:40+Math.random()*60,alpha:.04+Math.random()*.07,phase:Math.random()*Math.PI*2}));};
    resize();window.addEventListener("resize",resize);
    let t=0;
    const draw=()=>{const W=canvas.width,H=canvas.height;ctx.clearRect(0,0,W,H);t++;streams.forEach(s=>{const pulse=.6+Math.sin(t*.03+s.phase)*.4;const grad=ctx.createLinearGradient(s.x,s.y-s.len,s.x,s.y);grad.addColorStop(0,"rgba(0,0,0,0)");grad.addColorStop(1,`rgba(0,0,0,${s.alpha*pulse})`);ctx.strokeStyle=grad;ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(s.x,s.y-s.len);ctx.lineTo(s.x,s.y);ctx.stroke();ctx.globalAlpha=s.alpha*pulse*1.8;ctx.fillStyle="#111";ctx.beginPath();ctx.arc(s.x,s.y,1.5,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;s.y+=s.vy;if(s.y-s.len>H)s.y=-s.len;});id=requestAnimationFrame(draw);};
    ...
    <motion.div className="inline-flex items-center gap-5 border border-black/10 rounded-lg px-5 py-3"
      ...
      {metrics.map((m,i)=>(
        <div key={m.label} className="flex items-center gap-3">
          ...
          <div>
            <div className="text-base font-medium text-black tabular-nums">{i===0?counts[i].toLocaleString():counts[i]}{m.suffix??""}</div>
...
function ValueCard({item,index}:{item:typeof valueProps[0];index:number}) {
  ...
  return (
    <motion.div variants={staggerItem}
      ...
      whileHover={{y:-3,borderColor:"rgba(0,0,0,0.22)",boxShadow:"0 8px 32px rgba(0,0,0,0.07)",transition:{duration:.18,ease:EASE}}}
      onMouseEnter={onEnter} onMouseMove={onMove} onMouseLeave={onLeave}>
      <motion.div className="absolute inset-0 pointer-events-none rounded-lg" style={{opacity:glowOp,background:glowBg}} />
      <motion.div className="absolute top-0 pointer-events-none" style={{left:0,width:"55%",height:1,background:"linear-gradient(90deg,transparent,rgba(0,0,0,0.3),transparent)",x:shimT}} />
      ...
      <div className="relative">
        <div className="flex items-end justify-between mb-3">
          <div>
            <motion.div className="text-2xl font-medium text-black tabular-nums" animate={{scale:hovered?1.04:1}} transition={{duration:.2}}>{item.metric}</motion.div>
        ...
        <motion.div className="mt-3 h-px bg-black origin-left" animate={{scaleX:hovered?1:0}} transition={{duration:.4,ease:[.16,1,.3,1]}} />
      </div>
    </motion.div>
  );
}
...
function ServiceRow({label,desc,index}:{label:string;desc:string;index:number}) {
  ...
  return (
    <motion.div ref={rowRef} variants={staggerItem}
      ...
      <motion.div className="absolute inset-0 bg-black/[0.018]" initial={{scaleX:0,originX:0}} animate={{scaleX:hovered?1:0}} transition={{duration:.35,ease:[.16,1,.3,1]}} />
      ...
      <div className="md:col-span-1 pt-1 relative">
        <motion.span className="text-xs font-medium" animate={{color:hovered?"#111827":"rgba(0,0,0,0.2)"}} transition={{duration:.2}}>0{index+1}</motion.span>
      </div>
      ...
      <div className="md:col-span-8 relative"><p className="text-sm text-black/60 leading-relaxed">{desc}</p></div>
    </motion.div>
  );
}
    ...
    <div className="mt-8 bg-white border border-black/10 rounded-lg px-7 py-6">
      ...
      <div className="relative flex items-start justify-between">
        {steps.map((s,i)=>(
          <div key={s} className="flex flex-col items-center gap-2" style={{flex:1,position:"relative",zIndex:2}}>
            <motion.div className="w-9 h-9 rounded-full border flex items-center justify-center text-[9px] font-medium"
              animate={{background:activeStep===i?"#111827":"#fff",borderColor:activeStep===i?"#111827":"rgba(0,0,0,0.14)",color:activeStep===i?"#fff":"rgba(0,0,0,0.45)",boxShadow:activeStep===i?"0 0 0 5px rgba(0,0,0,0.08)":"none"}}
        ...
        <div ref={trackRef} className="absolute" style={{top:17,left:"5%",right:"5%",height:2,background:"rgba(0,0,0,0.07)",zIndex:1}}>
          <motion.div className="absolute top-0 left-0 h-full bg-black/20" animate={{width:`${(activeStep/(steps.length-1))*100}%`}} transition={{duration:.5,ease:"easeInOut"}} />
            ...
            {dataOfferings.map((o,i)=>{
              ...
              return (
                <button key={o.label} onClick={()=>setActive(i)}
                  ...
                  style={{background:isActive?"#111827":"transparent"}}>
                  <NavIcon size={15} strokeWidth={1.75} color={isActive?"#fff":"rgba(0,0,0,0.35)"} style={{flexShrink:0,transition:"color .18s"}} />
                  <span style={{fontSize:13,fontWeight:isActive?500:400,color:isActive?"#fff":"rgba(0,0,0,0.5)",transition:"color .18s",lineHeight:1.3}}>{o.label}</span>
                </button>
              );
            })}
          </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{opacity:0,x:12}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-12}}
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                {current.tags.map(t=>(
                  <span key={t} className="text-[11px] font-medium px-3 py-1 rounded-full border border-black/10 text-black/45 bg-black/[0.02]">{t}</span>
                ))}
              </div>
              ...
              <div className="flex items-center justify-between mt-8 pt-6 border-t border-black/[0.06]">
                <span className="text-xs text-black/30">{active+1} of {dataOfferings.length}</span>
      ...
      <section className="py-24 px-4 bg-black/[0.02] border-t border-b border-black/10">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-start">
          <div>
            ...
            <motion.h2 className="text-4xl md:text-5xl font-normal text-black leading-tight tracking-tight mb-6"
              variants={fadeUp} custom={.05} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
              Integration and data are two halves of the same job.
            </motion.h2>
            <motion.p className="text-base text-black/55 leading-relaxed"
              variants={fadeUpLarge} custom={.12} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
      ...
      <section className="py-24 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="max-w-2xl mb-12">
            ...
            <motion.h2 className="text-4xl md:text-5xl font-normal text-black leading-tight tracking-tight"
              variants={fadeUp} custom={.05} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
