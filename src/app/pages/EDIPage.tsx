...
// body-lg: text-lg          text-[#475569]  leading-relaxed
// body: text-sm             text-[#475569]  leading-relaxed
// label: text-xs            text-[#1A73E8]  uppercase tracking-wide
...
function WaveBackground() {
  ...
  useEffect(() => {
    ...
    const WAVES=[{amp:18,freq:.012,spd:.018,y:.42,a:.09,lw:1.8},{amp:12,freq:.018,spd:.024,y:.52,a:.06,lw:1.2},{amp:22,freq:.009,spd:.013,y:.60,a:.05,lw:2.2},{amp:9,freq:.025,spd:.031,y:.35,a:.04,lw:1.0},{amp:15,freq:.014,spd:.020,y:.70,a:.04,lw:1.5}];
    const draw=()=>{const W=canvas.width,H=canvas.height;ctx.clearRect(0,0,W,H);t++;WAVES.forEach(w=>{ctx.beginPath();ctx.moveTo(0,H*w.y);for(let x=0;x<=W;x+=2){const y=H*w.y+Math.sin(x*w.freq+t*w.spd)*w.amp+Math.sin(x*w.freq*1.6+t*w.spd*.7)*w.amp*.4;ctx.lineTo(x,y);}ctx.strokeStyle=`rgba(26,115,232,${w.a})`;ctx.lineWidth=w.lw;ctx.stroke();});id=requestAnimationFrame(draw);};
...
function DataParticles() {
  const ref=useRef<HTMLCanvasElement>(null);
  useEffect(()=>{const canvas=ref.current;if(!canvas)return;const ctx=canvas.getContext("2d")!;let id:number;type P={x:number;y:number;vx:number;vy:number;a:number;r:number};let pts:P[]=[];const resize=()=>{canvas.width=canvas.offsetWidth;canvas.height=canvas.offsetHeight;pts=Array.from({length:28},()=>({x:Math.random()*canvas.width,y:Math.random()*canvas.height,vx:(Math.random()-.5)*.35,vy:-Math.random()*.4-.1,a:Math.random()*.11+.04,r:Math.random()*2+1}));};resize();window.addEventListener("resize",resize);const draw=()=>{const W=canvas.width,H=canvas.height;ctx.clearRect(0,0,W,H);for(let i=0;i<pts.length;i++)for(let j=i+1;j<pts.length;j++){const dx=pts[i].x-pts[j].x,dy=pts[i].y-pts[j].y,d=Math.sqrt(dx*dx+dy*dy);if(d<120){ctx.globalAlpha=((120-d)/120)*.06;ctx.strokeStyle="#1A73E8";ctx.lineWidth=.8;ctx.beginPath();ctx.moveTo(pts[i].x,pts[i].y);ctx.lineTo(pts[j].x,pts[j].y);ctx.stroke();}}pts.forEach(p=>{ctx.globalAlpha=p.a;ctx.fillStyle="#1A73E8";ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fill();p.x+=p.vx;p.y+=p.vy;if(p.y<-4){p.y=H+4;p.x=Math.random()*W;}if(p.x<0||p.x>W)p.vx*=-1;});ctx.globalAlpha=1;id=requestAnimationFrame(draw);};draw();return()=>{cancelAnimationFrame(id);window.removeEventListener("resize",resize);};},[]);
  return <canvas ref={ref} className="absolute inset-0 w-full h-full pointer-events-none" style={{zIndex:0}} />;
}
...
function LifecycleRow({item,index}:{item:typeof lifecycle[0];index:number}) {
  ...
  return (
    <motion.div ref={ref} variants={staggerItem}
      className="grid md:grid-cols-12 gap-6 py-7 border-b border-[#0B1F3A]/10 items-start relative overflow-hidden cursor-default"
      ...
      <div className="md:col-span-3 relative">
        <motion.h3 className="text-sm font-medium" animate={{color:hovered?"#1A73E8":"#0B1F3A"}} transition={{duration:.2}}>{item.phase}</motion.h3>
...
function PlatformCard({name,logo}:{name:string;logo:string}) {
  ...
  return (
    <motion.div variants={staggerItem}
      className="border border-[#0B1F3A]/10 rounded-lg py-7 px-6 flex items-center justify-center relative overflow-hidden cursor-pointer"
      whileHover={{y:-3,borderColor:"rgba(26,115,232,0.3)",boxShadow:"0 6px 20px rgba(26,115,232,0.1)",transition:{duration:.18,ease:EASE}}}
...
function OutcomeCard({label,desc,metric,index}:{label:string;desc:string;metric:string;index:number}) {
  ...
  return (
    <motion.div ref={ref} variants={staggerItem}
      className="bg-white border border-[#0B1F3A]/10 rounded-lg p-8 space-y-3 relative overflow-hidden cursor-default"
      ...
      <motion.div className="text-3xl font-light tabular-nums text-[#1A73E8] tracking-tight"
        initial={{opacity:0,y:8}} animate={{opacity:shown?1:0,y:shown?0:8}} transition={{duration:.5,delay:index*.1}}>
        {metric}
      </motion.div>
      <h3 className="text-base font-medium text-[#0B1F3A] relative z-10">{label}</h3>
      <p className="text-sm text-[#475569] leading-relaxed relative z-10">{desc}</p>
      <motion.div className="h-px bg-[#1A73E8] origin-left relative z-10" animate={{scaleX:hovered?1:0}} transition={{duration:.4,ease:[.16,1,.3,1]}} />
    </motion.div>
  );
}
...
function DiffRow({heading,desc}:{heading:string;desc:string}) {
  ...
  return (
    <motion.div variants={staggerItem}
      className="py-7 border-b border-[#0B1F3A]/10 grid md:grid-cols-2 gap-6 relative overflow-hidden cursor-default"
      onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}>
      <motion.div className="absolute inset-0 bg-[#1A73E8]/[0.02]" initial={{scaleX:0,originX:0}} animate={{scaleX:hovered?1:0}} transition={{duration:.35,ease:[.16,1,.3,1]}} />
      <h3 className="text-sm font-medium text-[#0B1F3A] relative flex items-start gap-2 leading-snug">
        <motion.span className="text-[#1A73E8]/50 mt-0.5 flex-shrink-0" animate={{x:hovered?3:0,color:hovered?"#1A73E8":"rgba(26,115,232,0.5)"}} transition={{duration:.2}}>→</motion.span>
        {heading}
      </h3>
...
function OfferingCard({ icon: Icon, title, items, index }: typeof offerings[0] & { index: number }) {
  ...
  return (
    <motion.div variants={staggerItem}
      ...
      style={{ background:hov?"#0B1F3A":"#fff", borderColor:hov?"#0B1F3A":"rgba(11,31,58,0.1)", transition:"background .28s,border-color .28s,box-shadow .28s", boxShadow:hov?"0 16px 40px rgba(11,31,58,0.22)":"none" }}
      whileHover={{ y:-4, transition:{ duration:.2, ease:EASE } }}>
      <div style={{ position:"absolute", top:0, left:0, right:0, height:2, borderRadius:"8px 8px 0 0", background:"#1A73E8", opacity:hov?1:0, transition:"opacity .28s" }} />
      <div className="flex items-start gap-3 mb-4">
        <div style={{ width:38, height:38, borderRadius:9, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", background:hov?"rgba(255,255,255,0.1)":"rgba(26,115,232,0.08)", border:`1px solid ${hov?"rgba(255,255,255,0.15)":"rgba(26,115,232,0.15)"}`, transition:"background .25s,border-color .25s" }}>
          <Icon size={17} strokeWidth={1.75} color={hov?"#fff":"#1A73E8"} />
        </div>
        <h3 style={{ fontSize:13, fontWeight:500, lineHeight:1.35, color:hov?"#fff":"#0B1F3A", transition:"color .22s", flex:1 }}>{title}</h3>
      </div>
      <ul style={{ margin:0, padding:0, listStyle:"none", display:"flex", flexDirection:"column", gap:7 }}>
        {items.slice(0, open ? items.length : 3).map((item, i) => (
          <motion.li key={i} initial={{opacity:0,x:-6}} animate={{opacity:1,x:0}} transition={{duration:.28,delay:i*.04}}
            ...
            <span style={{ width:3, height:3, borderRadius:"50%", flexShrink:0, marginTop:6, background:hov?"rgba(255,255,255,0.3)":"rgba(26,115,232,0.35)", transition:"background .22s" }} />
            <span style={{ fontSize:12, lineHeight:1.6, color:hov?"rgba(255,255,255,0.55)":"rgba(71,85,105,0.95)", transition:"color .22s" }}>{item}</span>
          </motion.li>
        ))}
      </ul>
      {items.length > 3 && (
        <button onClick={e=>{e.stopPropagation();setOpen(o=>!o);}}
          style={{ marginTop:10, fontSize:11, fontWeight:500, color:hov?"rgba(255,255,255,0.4)":"#1A73E8", background:"none", border:"none", cursor:"pointer", padding:0, transition:"color .2s" }}>
    ...
    <section className="py-24 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="max-w-2xl mb-14">
          ...
          <motion.h2 className="text-4xl md:text-4xl font-normal text-[#0B1F3A] tracking-tight leading-tight" variants={fadeUp} custom={.05} initial="hidden" whileInView="visible" viewport={VIEWPORT}>EDI & B2B Integration <span className="text-[#1A73E8]">offerings</span> 
</motion.h2>
          <motion.p className="mt-4 text-base text-[#475569] leading-relaxed" variants={fadeUpLarge} custom={.12} initial="hidden" whileInView="visible" viewport={VIEWPORT}>10 specialised capabilities. 1 team. No handoffs between firms.</motion.p>
        </div>
              ...
              return (
                <button key={o.label} onClick={()=>setActive(i)}
                  ...
                  <NavIcon size={15} strokeWidth={1.75}
                    color={isActive ? "#fff" : "rgba(26,115,232,0.55)"}
...
function CTASection() {
  return (
    <section className="py-24 px-4 border-t border-[#0B1F3A]/10 relative overflow-hidden">
      <WaveBackground />
      <div className="max-w-6xl mx-auto text-center space-y-6 relative" style={{zIndex:1}}>
        <motion.h2 className="text-4xl md:text-5xl font-normal text-[#0B1F3A] tracking-tight"
          ...
          Find out exactly where <span className="text-[#1A73E8]">your EDI estate stands.</span>
        </motion.h2>
        ...
        <div className="max-w-6xl mx-auto relative" style={{zIndex:1}}>
          <div className="max-w-3xl space-y-6">
            ...
            <motion.h1 className="text-5xl md:text-[64px] font-normal text-[#0B1F3A] leading-[1.06] tracking-tight"
              ...
              EDI is not a legacy problem. <span className="text-[#1A73E8]">It's a competitive advantage — when it's done right.</span>
            </motion.h1>
            {/* body-lg */}
            ...
            <motion.p className="text-xs text-[#0B1F3A]/30"
      ...
      <section className="py-24 px-4 bg-[#1A73E8]/[0.02] border-t border-b border-[#0B1F3A]/10">
        <div className="max-w-6xl mx-auto">
          <div className="max-w-2xl mb-12">
            ...
            <motion.h2 className="text-5xl md:text-5xl font-normal text-[#0B1F3A] tracking-tight leading-tight"
              variants={fadeUp} custom={.05} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
              Everything needed to <span className="text-[#1A73E8]">build, run, and scale EDI.</span>
            </motion.h2>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="max-w-2xl mb-12">
            ...
            <motion.h2 className="text-4xl md:text-5xl font-normal text-[#0B1F3A] tracking-tight leading-tight"
              variants={fadeUp} custom={.05} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
              Connecting every platform. <span className="text-[#1A73E8]">Simplifying every process.</span>
            </motion.h2>
          </div>
          ...
          <motion.div className="border border-[#0B1F3A]/10 rounded-lg px-8 py-6 bg-[#1A73E8]/[0.02]"
            variants={fadeUp} custom={.1} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
            <p className="text-base text-[#0B1F3A] leading-relaxed">
              We modernize your stack, not sell you ours. Platform-agnostic by design, we recommend only what's right for your business.
            </p>
            <p className="mt-3 text-sm text-[#475569]">We support ANSI X12, EDIFACT, AS2, and API-led integration standards.</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 px-4 bg-[#1A73E8]/[0.02] border-t border-b border-[#0B1F3A]/10">
        <div className="max-w-6xl mx-auto">
          <div className="max-w-2xl mb-12">
            ...
            <motion.h2 className="text-4xl md:text-4xl font-normal text-[#0B1F3A] tracking-tight leading-tight"
              variants={fadeUp} custom={.05} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
              What great EDI looks like <span className="text-[#1A73E8]">in results ?</span>
            </motion.h2>
          </div>
      ...
      <section className="py-24 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="max-w-2xl mb-12">
            ...
            <motion.h2 className="text-4xl md:text-5xl font-normal text-[#0B1F3A] tracking-tight leading-tight"
              variants={fadeUp} custom={.05} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
              Built for enterprises that <span className="text-[#1A73E8]">expect more than good enough.</span>
            </motion.h2>
          </div>
          <motion.div className="space-y-0" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
