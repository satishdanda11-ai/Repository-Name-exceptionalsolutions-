    ...
    const draw = () => {
      ...
      for (let i = 0; i < nodes.length; i++)
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (d < 140) {
            ctx.globalAlpha = ((140 - d) / 140) * 0.06;
...
function InfraDiagram() {
  ...
  return (
    <div className="mt-8 bg-white border border-black/10 rounded-lg px-7 py-6">
      <p className="text-[9px] text-black/35 uppercase tracking-widest mb-6">Migration journey</p>
        ...
        {NODES.map((s, i) => (
          <div key={s} className="flex flex-col items-center gap-2" style={{ flex: 1, position: "relative", zIndex: 2 }}>
            <motion.div
              ...
              animate={{
                background:  activeNode === i ? "#111827" : "#fff",
                borderColor: activeNode === i ? "#111827" : "rgba(0,0,0,0.14)",
                color:       activeNode === i ? "#fff"    : "rgba(0,0,0,0.45)",
                boxShadow:   activeNode === i ? "0 0 0 5px rgba(0,0,0,0.08)" : "none",
              }}
              transition={{ duration: 0.32 }}>
              {i + 1}
            </motion.div>
            <motion.span className="text-[9px] text-center leading-tight"
              animate={{ color: activeNode === i ? "#111827" : "rgba(0,0,0,0.38)", fontWeight: activeNode === i ? 600 : 400 }}
              transition={{ duration: 0.25 }}>
              {s}
            </motion.span>
          </div>
        ))}
        <div ref={wrapRef} className="absolute" style={{ top: 17, left: "5%", right: "5%", height: 2, background: "rgba(0,0,0,0.07)", zIndex: 1 }}>
...
function UptimeRing({ value, label }: { value: number; label: string }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.strokeStyle = "rgba(0,0,0,0.07)"; ctx.lineWidth = 6; ctx.stroke();
      // fill
      const end = -Math.PI / 2 + progress * Math.PI * 2;
      ctx.beginPath(); ctx.arc(cx, cy, r, -Math.PI / 2, end);
      ctx.strokeStyle = "#111"; ctx.lineWidth = 6; ctx.lineCap = "round"; ctx.stroke();
      // value
      ctx.fillStyle = "#111"; ctx.font = `bold ${SIZE * 0.3}px system-ui`;
  ...
  return (
    <div ref={ref} className="flex flex-col items-center gap-2">
      ...
      <span className="text-[9px] text-black/45 uppercase tracking-wide text-center">{label}</span>
    </div>
  );
}
        ...
        <div className="max-w-6xl mx-auto relative" style={{ zIndex: 1 }}>
          <div className="max-w-3xl space-y-6">
            ...
            <motion.h1 className="text-5xl md:text-6xl font-normal text-black leading-tight tracking-tight"
              ...
              Cloud built for scale and reliability.
            </motion.h1>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-start">
          <div>
            ...
            <motion.h2 className="text-4xl font-normal text-black leading-tight tracking-tight mb-6"
              ...
              Modern integration belongs on modern infrastructure.
            </motion.h2>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="max-w-2xl mb-12">
            ...
            <motion.h2 className="text-4xl font-normal text-black leading-tight tracking-tight"
              variants={fadeUp} custom={0.05} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
              From migration to managed operations
            </motion.h2>
          </div>
