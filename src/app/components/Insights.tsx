        ...
        >
          {insights.map(({ title, description, category }) => (
            <motion.div
              ...
              className="bg-white border border-black/10 rounded-lg p-6 cursor-pointer overflow-hidden group"
              whileHover={{ y: -5, transition: { duration: 0.2, ease: EASE } }}
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-[10px] uppercase tracking-wider text-black/40">{category}</span>
              </div>

              <h3 className="text-base font-medium text-black mb-2 group-hover:text-black/70 transition-colors duration-200">{title}</h3>
              <p className="text-xs text-black/60 mb-4 leading-relaxed">{description}</p>

              <div className="text-xs text-black/60 group-hover:text-black transition-colors duration-200 flex items-center gap-1">
                Read More
