...
export function FinalCTA() {
  return (
    <section className="py-24 px-4 bg-black text-white relative overflow-hidden">
      ...
      {/* Animated gradient mesh — subtle, engineered feel */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div
          className="animate-mesh absolute -top-1/2 -left-1/4 w-[80%] h-[200%] opacity-[0.06] rounded-full"
          style={{
            background: "radial-gradient(ellipse at center, rgba(255,255,255,0.8) 0%, transparent 70%)",
          }}
        />
        <div
          className="animate-mesh absolute -bottom-1/4 -right-1/4 w-[60%] h-[150%] opacity-[0.04] rounded-full"
          style={{
      ...
      <div className="max-w-4xl mx-auto text-center space-y-6 relative z-10">
        ...
        <motion.div
          ...
          viewport={VIEWPORT}
        >
          {/* Primary — with soft pulse every 8s */}
          <motion.div whileHover={{ y: -3 }} transition={{ duration: 0.18, ease: EASE }}>
            <Link
              to="/contact"
              className="animate-btn-pulse block bg-white text-black px-6 py-2.5 text-sm rounded-md hover:bg-white/90 transition-colors"
            >
